...
public class ManaController {
    ...
    public ResponseEntity<Map<String, Object>> getStrat() {
        ...
        try {
            ...
            queryString = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>"
                ...
                +"SELECT DISTINCT (str(?manacost) as ?ManaCost)"
                +"WHERE {?x rdf:type hs:ManaCost."
                +" ?x rdfs:label ?manacost."
                +"}"
                +"ORDER BY ?x";
            ...
            while (results.hasNext()) {
                ...
                String Card = soln.getLiteral("ManaCost").getString();
                // test --
                System.out.println("ManaCost " + Card.toString());
    ...
    @GetMapping("/mana/new/{name}")
    public ResponseEntity<Map<String, Object>> postRare(@PathVariable Long name) {
        HashSet<String> ListComponent = new HashSet<String>(); // for name list
        ArrayList<String> heroList = new ArrayList<String>(); // for name list
        try {
            // OntModel model = OpenOWL.OpenConnectOWL();
            ...
            queryString = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>"
                ...
                +"PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>"
                +"PREFIX hs: <http://www.semanticweb.org/asus/ontologies/2019/1/untitled-ontology-18#>"
                +"SELECT (str(?card) as ?Card) (str(?mana) as ?Mana)"
                +"WHERE {{?x rdf:type hs:HearthStone_Card;"
                +"hs:hasManaCost ?y."
                +"?y rdfs:label ?mana."
                +"?x rdfs:seeAlso ?card."
                +"}FILTER (?mana = "+name+")}";
